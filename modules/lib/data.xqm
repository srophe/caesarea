...
declare function data:search($collection as xs:string*, $queryString as xs:string?, $sort-element as xs:string?) {                      
    ...
    return
            ...
            if($collection = 'bibl') then 
                ...
                order by global:build-sort-string(data:add-sort-options-bibl($hit, request:get-parameter('sort-element', '')),'')
                return $hit
            else 
                for $hit in $hits/ancestor-or-self::tei:TEI
            ...
            if($collection = 'bibl') then 
                for $hit in $hits/ancestor-or-self::tei:TEI
                order by global:build-sort-string(data:add-sort-options-bibl($hit, $sort-element),'')
                return $hit
            else 
                for $hit in $hits/ancestor-or-self::tei:TEI
...
declare function data:add-sort-options($hit, $sort-option as xs:string*){
    if($sort-option != '') then
        (: Specieal fields for Caeserea testimonia module:) 
